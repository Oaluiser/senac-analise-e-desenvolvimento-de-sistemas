...
function App() {
  ...
  function excluiCarro(carro) {
    ...
    localStorage.setItem(
      "carros",
      JSON.stringify(carros.filter((c) => c !== carro))
    )
  }

  function filtrar() {
    setCarros(
      JSON.parse(localStorage.getItem("carros")).filter(
        (c) => c.modelo === watch("pesquisaModelo")
      )
    )
  ...
  return (
    <div className="container-fluid">
      ...
      >
        <div className="card">
          <div className="card-header">Inclusão do Carro</div>
          <form
            className="card-body"
            onSubmit={handleSubmit(gravaDados)}
          >
        ...
        <div className="card">
          ...
          <div className="card-body">
            ...
            <p className="card-text">
              {(
                carros.reduce((acc, cur) => acc + parseFloat(cur.preco), 0) /
                carros.length
              )
                .toFixed(2)
                .replace(".", ",")}
              ...
              {carros.filter(
                (c) =>
                  ...
                  carros
                    .reduce(
                      (acc, cur) =>
                        acc > parseFloat(cur.preco)
                          ? acc
                          : parseFloat(cur.preco),
                      0
                    )
                    .toString()
              )[0]?.modelo
                ? `Modelo: ${carros.filter((c) => c.preco === carros.reduce((acc, cur) => (acc > parseFloat(cur.preco) ? acc : parseFloat(cur.preco)), 0).toString())[0].modelo}`
            ...
            <p className="card-text">
              {`Valor: ${carros
                .reduce(
                  (acc, cur) =>
                    acc > parseFloat(cur.preco) ? acc : parseFloat(cur.preco),
                  0
                )
                .toFixed(2)
                .replace(".", ",")}`}
