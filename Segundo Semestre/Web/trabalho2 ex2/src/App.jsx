...
function App() {
  ...
  function excluiViagem(viagem) {
    ...
    localStorage.setItem(
      "viagens",
      JSON.stringify(viagens.filter((v) => v !== viagem))
    )
  }
  ...
  function alteraPreco() {
    localStorage.setItem(
      "viagens",
      JSON.stringify(viagens.filter((v) => v !== viagemAtual))
    )
  ...
  function filtrar() {
    setViagens(
      JSON.parse(localStorage.getItem("viagens")).filter(
        (v) => v.destino === watch("pesquisaDestino")
      )
    )
  ...
  const listaViagens = viagens.map((viagem, i) => (
    <tr key={i}>
      <td>
        <img
          src={viagem.foto}
          alt={`Foto do carro ${viagem.destino}`}
          width={150}
          height={100}
        />
      </td>
      ...
      <td>
        <i
          className="bi bi-airplane fs-4 text-danger ms-2"
          style={{ cursor: "pointer" }}
          title="Consulta Viagem"
          onClick={() => consultaViagem(viagem)}
        ></i>
        <i
          className="bi bi-pencil-square fs-4 text-danger ms-2"
          style={{ cursor: "pointer" }}
  ...
  return (
    <div className="container-fluid">
      ...
      >
        <div className="card">
          <div className="card-header">Inclusão da Viagem</div>
          <form
            className="card-body"
      ...
      >
        <div className="card">
          <div className="card-header">Detalhes da Viagem</div>
